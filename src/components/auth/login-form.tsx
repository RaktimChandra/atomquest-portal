...
import { ArrowRight, Loader2, Eye, EyeOff, Shield, UserCog, User2, Sparkles, type LucideIcon } from 'lucide-react';
...
const demoUsers: { role: string; email: string; name: string; icon: LucideIcon }[] = [
  { role: 'Admin / HR', email: 'admin@atomberg.com',    name: 'Priya Sharma',  icon: Shield  },
  { role: 'Manager',    email: 'manager@atomberg.com',  name: 'Rohan Kapoor',  icon: UserCog },
  { role: 'Employee',   email: 'employee@atomberg.com', name: 'Arjun Patel',   icon: User2   },
];
        ...
        >
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center text-[11px] uppercase tracking-[0.18em]">
              <span className="bg-background px-3 text-muted-foreground">or with credentials</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Magnetic strength={0.2}>
              <Button type="submit" variant="gradient" size="lg" className="w-full gap-2 btn-shimmer" disabled={loading}>
        ...
        >
            ...
            {demoUsers.map((u, i) => {
              ...
              return (
                <motion.button
                  ...
                  className="w-full text-left card-3d p-4 group"
                >
                  <div className="flex items-center gap-4">
                    <div className="h-11 w-11 rounded-xl bg-brand flex items-center justify-center shadow-md shadow-blue-500/20">
          ...
          <div className="card-3d p-4">
            <div className="text-xs text-muted-foreground space-y-1.5">
              <div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Audit-trail enabled on all actions</div>
              <div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-blue-500" /> Role-based access strictly enforced</div>
              <div className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-cyan-500" /> Sample cycle + thrust areas pre-seeded</div>
            </div>
          </div>
        </motion.div>
      </div>
